...
当前桌面发行版本为 `0.5.2`；`package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 与 `src-tauri/tauri.conf.json` 必须同步递增。本版本增加持久化周期类型、Runtime 能力与配置修订快照、模型调用前能力 fail-fast、缺失 verdict 度量，以及预算耗尽时的结构化阻断结果。
