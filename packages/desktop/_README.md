...
当前桌面发行版本为 `0.5.1`；`package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 与 `src-tauri/tauri.conf.json` 必须同步递增。本版本修复桌面后台只续租 Run、未续租活动 RuntimeBinding 的问题；长时间网络等待或模型推理可持续跨越多个 TTL，真正失去执行权时仍由 fencing 阻止迟到提交。
