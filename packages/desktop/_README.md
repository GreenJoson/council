...
当前桌面发行版本为 `0.9.4`；`package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 与 `src-tauri/tauri.conf.json` 必须同步递增。本版本增加失败任务的未提交代码接续：校验原工作区并过滤敏感内容后，将草稿带入新隔离运行，原文件与失败历史保留，仍经过执行、提交、审核和验收。人工验收、活动委派关闭保护在 Node 和 Rust 侧一致；详见 [执行交付](../../docs/execution-delivery.md)。
