...
当前桌面发行版本为 `0.4.6`；`package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 与 `src-tauri/tauri.conf.json` 必须同步递增。本版本在过长议题说明底部提供“展开议题 / 收起议题”，短议题保持原样；长消息卡仍只保留正文底部入口，图片/Mermaid 大图与一卡一节点的阶梯导航保持不变。
