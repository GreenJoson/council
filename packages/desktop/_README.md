...
当前桌面发行版本为 `0.5.5`；`package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 与 `src-tauri/tauri.conf.json` 必须同步递增。本版本为 Council ToolLoop 与 Kimi Code ACP 增加同一受控 Git diff 边界：只读取已提交 commit/ref，先过滤敏感路径，禁用外部驱动并限制输出；仍不开放未提交工作区、Shell 或写操作。
