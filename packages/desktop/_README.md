...
当前桌面发行版本为 `0.9.7`；`package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 与 `src-tauri/tauri.conf.json` 必须同步递增。本版本修正委派历史与正文缩进，并让展开的执行证据占满卡片宽度；保留显式接续权限和 OAuth 过期分类。继续使用 schema v17、分阶段预算与私有检查点，原记录不改写；桌面 sidecar 生命周期未改变。详见 [执行交付](../../docs/execution-delivery.md)。
