...
当前桌面发行版本为 `0.5.11`；`package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 与 `src-tauri/tauri.conf.json` 必须同步递增。本版本让圆桌自动冻结并复用提案人已有 proposal/开场 brief，直接从首位评审开始；已提交修复互审缺少真实 commit 时零调用拒绝，并把模式标题与说明改为上下两行紧凑布局。
