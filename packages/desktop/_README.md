...
当前桌面发行版本为 `0.9.6`；`package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 与 `src-tauri/tauri.conf.json` 必须同步递增。本版本补齐显式接续权限（默认保留原权限，受 Agent 当前上限约束）和 OAuth 过期终止原因分类。继续使用 schema v17、分阶段预算与私有检查点，原记录不改写；桌面 sidecar 生命周期未改变。详见 [执行交付](../../docs/execution-delivery.md)。
