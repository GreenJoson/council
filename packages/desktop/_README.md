...
当前桌面发行版本为 `0.4.8`；`package.json`、`package-lock.json`、`src-tauri/Cargo.toml`、`src-tauri/Cargo.lock` 与 `src-tauri/tauri.conf.json` 必须同步递增。本版本在讨论时间线增加按议题隔离的 Agent 在线状态和实时草稿：Claude 与兼容 Provider 转发公开文本增量，Codex 转发公开 JSONL 消息并平滑展开较大输出块；临时草稿不写 SQLite，完成后由正式消息接替。
