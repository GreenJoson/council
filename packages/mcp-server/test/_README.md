...
| 文件名 | 地位 | 功能 |
|---|---|---|
| `schema-migrator.test.ts` | 主安全验收 | 验证连续版本镜像、canonical schema、实例身份、在线备份及只读校验、空库转既有库重试、事务回滚、WAL 阻塞和未来版本拒绝 |
| `database.test.ts` | 单元测试 | 验证预迁移共享存储、revision 分域与 lease 零噪声 |
