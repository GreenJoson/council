...
| 文件名 | 地位 | 功能 |
|---|---|---|
| `app.ts` | 核心 | 组装内容/编排/RuntimeBinding/Model Router REST 路由、ready 状态、安全中间件与脱敏异常处理 |
| `config.ts` | 配置 | 用 Zod 校验迁移重试、HTTP、双 lease、sweeper、空闲绑定和 Agent 清理预算 |
| `schemas.ts` | 边界 | 严格校验 path、query、RuntimeBinding 动作、单次完成复核与 Agent 设置，拒绝伪造 Actor 身份 |
