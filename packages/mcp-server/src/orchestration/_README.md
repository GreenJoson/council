...
V1 只注册 `claude` 后台适配器。适配器不调用 `ClaudeClient.ask`，因此不会提前写消息；
回复只能由 `SQLiteCouncilStore.commitRound` 在 lease 和运行版本校验通过后原子发布。
V1 也不恢复 Claude session，避免把兼容层的双写语义带入编排。
...
生产初始化会检查 Claude Code CLI 与登录状态。不可用适配器在 capabilities 中标记
`available=false`，只返回固定限制说明，并在创建运行时被拒绝；底层本机错误不会进入响应。
