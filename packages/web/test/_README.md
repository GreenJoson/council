...
| `orchestration-repository.test.ts` | 编排测试 | 验证 policy 解析、双 revision 分流、Agent 草稿顺序/议题隔离、选题竞态、审批重放和 timer 生命周期 |
...
| `webui-smoke.py` | 浏览器测试 | 验证自动轮次、时间线实时草稿、议题隔离、大屏讨论列、移动端抽屉、无横向滚动和控制台错误 |
