...
| 文件名 | 地位 | 功能 |
|---|---|---|
| `useExecutionRepository.ts` | 依赖注入 | 由 App 共享现有编排仓储，供审计、待处理和恢复视图使用，不重复创建连接 |
...
| `useWorkItemDelegations.ts` | 委派状态 | 按当前议题加载 Agent 委派账本，复用 orchestration revision/SSE 刷新进度，并封装启动、取消与忙碌状态 |
