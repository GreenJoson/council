...
| `Composer.tsx` | 写入 | 发布带类型的公开回复（支持 ⌘Enter）、承接引用回复种子并聚焦编辑器、显示同步状态并在失败时保留草稿；输入 `@` 弹出候选下拉（数据取自编排能力 adapters，键盘上下选择、Enter/Tab 确认、Esc 关闭，不可用 Agent 灰显并展示限制文案），命中合法 @召唤 时把发布消息的 kind 强制改写为 note（召唤指令本身只是行为触发，不是真正的提案/结论），并在发布成功后额外触发一轮 orchestrationRepository.createRun + startRun；离线或该议题已有活动 Run 时在发布前用行内提示拦截（复用 AutoRoundsPanel 导出的 `getCreateRunBlockedReason`，文案保持一致） |
