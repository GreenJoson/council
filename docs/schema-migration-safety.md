...
当前 schema 版本为 `17`。版本 2 引入动态 Actor Identity；v3–v5 拆分 Provider/Agent、
...
v17 为委派追加私有执行检查点，不改旧状态和审计历史。v14→v17、v16→v17 的故障注入验证事务回滚后旧结构可继续迁移，Rust 同步验证 v17；旧二进制不得打开新版本库。
