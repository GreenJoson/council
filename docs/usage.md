...
- 既有提案复用：若首位提案人已发布 `proposal`，或它创建议题时发布了开场 `brief`，
  Council 会把该消息冻结成第一轮提案，直接从第二位 Agent 的 `critique` 开始；普通补充
...
- bug 修复互审：只用于**已经提交的修复**。修复必须先由 Codex App、Claude Code 等交互式开发任务完成并提供真实
  ...
  获得修改文件、写 Shell、运行写操作、创建提交、推送或部署权限。提案中没有真实
  `council-fix` commit 时会在调用模型前拒绝，不会再浪费一次提案人调用。
