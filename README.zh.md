...
6. **凭证据验收。** 查看审核和执行记录。默认策略要求你填写本次验收依据后完成任务。失败或取消的委派若存在有效提交，可在新工作区恢复，并先复审已有成果。
...
**恢复与证据。** 运行时事件经脱敏后持久化。恢复前检查已记录提交、同一仓库、受管目录、工作区干净状态、权限和任务版本，再创建新运行。没有提供的工具细节明确显示不可用；上下文占用不会被当成计费 Token 或推算费用。
...
- 新委派默认人工验收。写入失败和未提交改动不会被自动重放；没有提交的工作无法由检查点恢复重建。
